...
const FAQ = () => {
    ...
    return ( 
        <main className="relative px-[5%] py-16 pb-24 grid grid-cols-1 gap-4 items-center justify-center place-items-center bg-white text-[16px] text-dark">
            <h1 className="text-[46px] font-extrabold text-center">Frequently Asked Questions</h1>
            <p className="leading-relaxed tracking-wide font-light text-center max-w-[80%]">If you are not sure weather draft is suitable for you or not, do not worry. We are here to explain everything you might want to know</p>
            ...
            <section className="grid gap-4 w-[80%]">
                ...
                <div key={index} className="grid gap-4 p-4 rounded-xl border shadow-xl">
                    <div className="flex items-center justify-between">
                        <h3 className="text-[18px]">{faqs.question}</h3>
                    ...
                    {expandedIndex === index && (
                        <p className="text-[15px]">{faqs.answer}</p>
                    )}
                </div>
                ))}
            </section>
